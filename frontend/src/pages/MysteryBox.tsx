  ...
  return (
    <div className="min-h-screen bg-brutal-bg flex flex-col">
      ...
      <main className="flex-grow max-w-6xl mx-auto px-4 py-16 w-full">
        ...
        <div className="grid md:grid-cols-3 gap-12">
            {boxes.map((box) => (
                <BrutalCard key={box.id} accent="black" className={`${box.color} p-8 flex flex-col items-center gap-6 group hover:-translate-y-4 transition-all`}>
                    ...
                    <BrutalButton variant="black" className="w-full py-4" onClick={() => handleSpin(box)} disabled={isSpinning}>
        ...
        <AnimatePresence>
            {isSpinning && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-brutal-black/90 flex flex-col items-center justify-center p-4 backdrop-blur-xl">
                    <motion.div animate={{ rotate: [0, -10, 10, -10, 10, 0], scale: [1, 1.2, 1.2, 1.2, 1.2, 1] }} transition={{ duration: 3, ease: "easeInOut" }} className="text-[12rem]">{selectedBox?.icon}</motion.div>
                    <h2 className="text-4xl font-space font-black text-white uppercase italic animate-pulse mt-8">Sedang Membuka Box...</h2>
                </motion.div>
            )}

            {reward && (
                <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="fixed inset-0 z-[201] flex items-center justify-center p-4 bg-brutal-magenta/20 backdrop-blur-sm" onClick={() => setReward(null)}>
